...
if(in_array($filecheck, $fileextstored)){
    ...
    $hashedPassword = password_hash('1234', PASSWORD_DEFAULT); // Use a secure hashing algorithm

$sql = "INSERT INTO `employee`(`id`, `firstName`, `lastName`, `email`, `password`, `birthday`, `gender`, `contact`, `nid`,  `address`, `dept`, `degree`, `pic`) VALUES ('',?,?,?,?,?,?,?,?,?,?,?,?)";
$stmt = mysqli_prepare($conn, $sql);

mysqli_stmt_bind_param($stmt, 'ssssssssssss', $firstname, $lastName, $email, $hashedPassword, $birthday, $gender, $contact, $nid, $address, $dept, $degree, $destinationfile);
